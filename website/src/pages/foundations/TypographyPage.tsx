...
export default function TypographyPage() {
  return (
    <div>
      <PageHero
        title="Typography"
        lead={<>USPACE 使用 <strong style={{ color: 'var(--text-secondary)' }}>PingFang TC</strong>（中文）與{' '} <strong style={{ color: 'var(--text-secondary)' }}>SF Pro</strong>（英數）兩套字型系統。 fontWeight 必須完全以 JSON 數值為準（400=Regular, 500=Medium, 700=Bold），不依語意推斷。 captionS 和 displayM 的 fontSize 在兩套字體中刻意不同，這是設計意圖。</>}
      />
