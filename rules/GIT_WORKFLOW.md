...
gh pr create                           # 4. 開 PR
# 5. 等 CI 兩個 job（flutter、web）都綠 → 到這裡為止
```

**每一項工作都開新分支，即使當下沒有其他分支在跑。** 直接在 main 上做，
協作者就沒有可以對照的基準點。

### 合併由每日同步統一進行

**CI 綠之後停下來，不要自己合併。** PR 開著等每日同步時段，由維護者
一次合併當天所有綠燈的 PR。這樣 main 一天只動一次，協作者有一個
固定、可預期的同步點，不會今天拉一次明天又落後。

CI 紅燈時停下來修，修好再等下一次同步。

合併之後才會部署，所以改動要到同步時段過後才看得到線上效果。急件
請直接說「這個先合併」，不必等。

> **這個做法有代價，要一起做兩件事才有效**：分支開著愈久，跟 main
> 的差距愈大，衝突反而愈多。所以（一）每個 PR 儘量小、當天開當天
> 就緒；（二）每次開分支前務必先 `git pull`——固定同步點只有在
> 大家都從同一個起點出發時才有意義。
