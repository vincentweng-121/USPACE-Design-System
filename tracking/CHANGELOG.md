...
- 勾叉記號不再疊在圖片右下角。改為圖片下方接一列「記號＋Do／Don't」標題，
  再接說明文字，整組靠左對齊。
- 記號縮小為 20（原本疊在圖上時是 32），與標題文字等高。
- 說明文字由置中改為靠左。`.note` 本身仍是置中，只有這裡覆寫。
- Edge cases 的圖例沒有 kind，只會出現說明文字，沒有標題列。

### 網站樣式 | 勾與叉改用使用者指定的顏色
狀態：PUBLISHED

- `--positive` 由 #17803D 改為 **#A7D100**，`--negative` 由 #C22B2B 改為 **#F40000**。
- 這兩個值都是 palette 既有的顏色：#A7D100 是 neonLime800、#F40000 是 red500。
  目前產生器只輸出 shadow 一個 CSS 變數，沒有 palette 變數可以引用，
  所以先寫在 global.css 並註明對應關係。
- 深色模式改為同一組值，不再另外用淺綠與淺紅——記號不是文字，
  在深底上的對比仍然足夠。
...
### 文件站 | Button 說明圖重新匯出，16 張有變動
狀態：PUBLISHED

- 從 Figma section 3726:8888（Button-MCPimages）以 scale 2 重新匯出全部 24 張，
  節點編號與上次相同。比對後 16 張有變動，其餘 8 張位元組完全相同。
- 多數變動是底色調整（light 版幾乎全部重繪），量測數值沒有改變：
  高度 48 / 40、icon 24、間距 8、水平內距 12，與 v0.16.6 一致。
- **do-case3 / dont-case3 換了示範內容**：現在的 Do 是「保留車位」純文字按鈕，
  Don't 是只有勾勾 icon 的按鈕。圖說隨之改寫——原本寫「icon 一律搭配文字」，
  但新的 Do 圖裡根本沒有 icon。

### 文件站 | 修正 Anatomy 表格殘留的舊說法
狀態：PUBLISHED

- 「容器 Container」那一列還寫著「底色與圓角由 level 決定；三個層級皆無描邊」。
  Button 在 v0.15.0 已經移除 level 這個維度，outlined 也有 3px 漸層描邊。
- 改為依 style 描述，高度改為引用 token 而非寫死的 48。
...
- `.content` 的左右內距全部加倍：桌機 40 → 80、1200 以下 28 → 56、
  900 以下 20 → 40。
- 首頁的 `.home-body` 維持原值，它是滿版影片首屏的獨立版面。
...
- 搜尋面板：改為固定高度（70vh 上限 480），底部的「↑↓ 移動／↵ 開啟」
  說明固定停在容器最下方，不再隨結果多寡上下移動。
- 搜尋觸發鈕：移除輸入框外觀（底色、陰影、「搜尋」文字、⌘K 標籤），
  改用與其他頂列按鈕相同的純 icon 樣式。⌘K 快捷鍵不變。
- 主題切換：移除「跟隨系統」模式，只留淺色與深色兩種，點一下就切換。
  原本是三段循環，切到「跟隨系統」時外觀常與前一個模式相同，
  看起來像沒反應，得再點一次。舊使用者存的 'auto' 會依當下系統偏好轉為
  light 或 dark。
- 頂列與左側抽屜移除陰影（首頁浮動頂列與窄螢幕抽屜的覆蓋陰影一併移除）。
- 僅為文件網站的呈現調整，元件程式碼與 token 未變動。
...
### 文件網站 | Button 頁 States 容器加大並垂直置中
狀態：PUBLISHED

- Enabled 與 Disabled 之間的水平間距由 48 拉開為 96。
- 容器最小高度設為 288，約為原本自動高度的兩倍。
- 內容在容器內水平與垂直都置中。
- 只調整 Button 頁。Dropdown Menu 頁的 States 有六個狀態並排，
  加倍間距會被迫換行，維持原本的 32。
- 僅為文件網站的呈現調整，元件程式碼與 token 未變動。
...
**排版**
- Do/Don't 的卡片間距由 16 加倍為 32、組與組之間由 32 加倍為 64、
  圖與說明由 12 加倍為 24。圖片隨欄寬等比縮小，整組左右仍對齊內容邊界。
- 說明文字改為置中對齊它上方的圖。
- Edge cases 套用同一套間距與圖說規則。`AnatomyImage` 新增 `flush`，
  由外層 gap 控制間距時不再出內建的下留白，其他頁面的用法不受影響。

**表格**
- 移除陰影（含 Roadmap 的甘特圖）。
- 兩種底色都與 body 不同：`page-secondary` 與 `surface-sunken`，明暗皆成立，
  靠色差就分得出範圍與列。
- 表頭由置中改為置左，與下方欄位對齊。

**說明文字全站統一**
- 新增 `.note`（段落：13px、`textTertiary`、置中）與 `.note-list`
  （條列：同字級色階，維持靠左——置中會讓項目符號錯位而難讀），
  取代原本散在 48 處的 inline 樣式。
- Markdown 渲染的內文維持原樣：那是文章正文，不是區塊說明。

**陰影**
- 稽核時發現 `PageTabs` 有兩處自訂陰影未納入 v0.15.2 的整併，已處理。
- 分頁列改用新的 `--shadow-tabs`（blur 15、透明度 5%，使用者指定）。
  ⚠️ 這個值還沒有對應的設計系統 token，已在 CSS 註明：要沿用到其他元件前
  應先在 Figma 定義，不要從那裡複製數值出去。

**其他**
- Do/Don't 的勾與叉改為實心。
- 移除 Edge cases 的導言，以及 States 的兩處說明文字。
...
### 文件站 | 側欄隱藏捲軸、Do/Don't 圖例改為記號覆蓋
狀態：PUBLISHED

- **左側側欄隱藏捲軸外觀**，捲動行為不變（`scrollbar-width: none` 加上
  `::-webkit-scrollbar { display: none }`）。
- **Do/Don't 圖例移除下方的說明區塊**：原本圖片下方有一塊帶底色、含 icon 與
  「Do」/「Don't」標籤的區塊，現在只留說明文字，用一般的次要文字色。
  是 Do 還是 Don't 改由圖上的記號表達，不再重複講一次。
- **圖片外框移除**，改用與全站一致的 `--shadow-card` 陰影。
- **勾與叉改為覆蓋在圖片右下角**，尺寸由 16 放大為 32，綠色與紅色不變。

Usage 區塊裡另一種用法（`DoDont` 的條列清單）不受影響，那裡的 icon 維持 16。
...
參考 Apple Vision Pro 的做法：用浮起感分區，而不是畫線。

- 新增兩個陰影變數 `--shadow-card` 與 `--shadow-card-sm`（明暗各一套；
  深色底下陰影本身看不出來，改用更深的陰影拉開層次）。
- **表格**：外框與所有格線移除，改為圓角 + 陰影；列與列改用極淡的交替底色區分。
  甘特圖同樣處理，週次的直向格線也改成交替底色。
- **區塊容器**：Playground、SpecBox、AnatomyFigure、CodeBlock、DoDont、
  ConfidenceNote、說明圖容器、各頁的卡片等 23 處的 1px 外框全部改為陰影。
- **網站外框**：頂列底線、側欄右線、搜尋面板外框與其內部分隔線、
  SOON 徽章外框、搜尋觸發器外框一併移除。
- **色票方塊**：改用最輕的陰影而不是邊線——白色色票在白底上仍要看得見。
- 行內 `code` 的細框移除，靠底色辨識。
- blockquote 的左側粗線改為底色區塊。
- Overview 頁原本用「1px 間隙 + 底色」假造的格線，改為留白。

#### 刻意保留

- **兩條導覽指示線**（左側選單與右側 On this page）——使用者指定保留。
- **元件自己的 divider**：List 與 Modal 的分隔線、TextField / TextArea 的
  輸入框描邊、Action Area 預覽裡 outlined 按鈕的描邊。這些是元件規格的一部分，
  不是網站的裝飾。
- **虛線佔位框**（Pending / PendingImage / Coming soon / Patterns，以及
  Action Area 的 none 背景範圍）：它表達的是「這裡還沒有內容」，
  不是分隔線；移除後讀者看不出範圍。
